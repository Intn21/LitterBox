...
| [prepare_shakespeare.py](prepare_shakespeare.py) | Fetch Tiny Shakespeare and split it 90/10 for `configs/data/tinyshakespeare.yaml`. With `configs/{models,training}/shakespeare-char.yaml` this is nanoGPT's character-level recipe on this repo's loop: 10.7M parameters, one epoch every 61 steps, about 80 minutes for the full 5,000 steps on an Apple M4 |
...
| [init_checkpoint.py](init_checkpoint.py) | Save a step-0 checkpoint with random weights into a run directory, so `generate.py` (and a resumed training run) can start from it. The before picture, and a fixed init two runs can share |
